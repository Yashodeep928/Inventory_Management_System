...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Submit to backend with correct DB field mapping
    // UI: username -> DB: name
    try {
      setSubmitting(true);
    ...
    <main className="flex flex-col lg:flex-row justify-start items-center w-full min-h-screen bg-[#ffffff]">
      ...
      <section className="flex flex-col justify-start items-end w-full lg:w-[42%] px-[28px] sm:px-[42px] lg:px-[56px] pr-[47px] sm:pr-[70px] lg:pr-[94px] py-8 lg:py-0">
        ...
        >
          ...
          {/* Username (maps to DB 'name') */}
          <Input
            type="text"
            placeholder="Username"
            value={form.username || ""}
            onChange={(e) =>
              dispatch(updateForm({ field: "username", value: e.target.value }))
            }
          ...
          <Button variant="secondary" size="md" className="w-full relative" disabled={submitting}>
            <img
              src="/images/img_google_1.png"
              className="absolute left-[21px] w-[30px] h-[30px]"
              alt="Google logo"
            />
